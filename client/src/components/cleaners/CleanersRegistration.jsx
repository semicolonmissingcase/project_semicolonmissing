...
export default function CleanersRegistration() {
  ...
  // --- 3. useMemo (파생된 데이터/옵션 계산) ---

  // 🟢 [정리] 시/도 옵션 생성 및 군/구 맵 생성 로직 통합
  const memoizedOptions = useMemo(() => {
    const locationArray = locations.rows || [];

    console.log('DEBUG 1: Extracted locationArray length:', locationArray.length);
  ...
  const handleCityChange = (selectedOption) => {
    // 1. 선택된 시/도 이름을 상태에 저장
    setSelectedCity(selectedOption ? selectedOption.value : '');
    // 2. 시/도를 변경하면 구/군 선택 목록이 자동으로 업데이트됩니다.
  };
  ...
  // 구/군 토글 핸들러 (location 객체 { value: id, label: district, ... } 사용)
  const handleLocationToggle = (location) => {
    const locationId = location.value; // ID를 사용
  ...
  const handleSubmit = async (e) => {
    ...
    console.log('locations length:', formData.locations.length);
    console.log('---------------------');
      ...
      <div className="cleaners-registration-section">
        ...
        <div className="cleaners-registration-form-group">
          ...
          <Select
            ...
            options={cityOptions} // ⬅️ memoizedOptions.cityOptions 사용
