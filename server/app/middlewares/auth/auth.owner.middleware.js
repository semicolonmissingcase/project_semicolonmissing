 ...
 */
function ownerAuthenticate(req) {
  // 토큰 획득
...
function ownerAuthorize(req) {
  ...
  if(matchRole) {
    ...
    ownerAuthenticate(req);

    // 권한 체크
    const ownerRole = req.owner?.role;
    if(!ownerRole || !matchRole.roles.includes(ownerRole)) {
