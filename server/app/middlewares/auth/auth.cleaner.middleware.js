/**
 * @file /app/middlewares/auth/cleanner.middleware.js
 * @description 기사 인증 및 인가 처리 미들웨어
 ...
 */
function cleanerAuthenticate(req) {
  // 토큰 획득
...
function cleanerAuthorize(req) {
  ...
  if(matchRole) {
    ...
    cleanerAuthenticate(req);

    // 권한 체크
    const cleanerRole = req.owner?.role;
    if(!cleanerRole || !matchRole.roles.includes(cleanerRole)) {
